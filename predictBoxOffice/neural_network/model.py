...
class MLP(object):
    def __init__(self, sess, name="default", input_dim=20, output_dim=10, hidden_depth=5, stddev=0.02,
    ...
    def run1(self, steps=20000, name="", is_save=False):
        saver = tf.train.Saver()
        if is_save:
            saver.restore(self.sess, save_path="save/hellokugou_" + name + ".ckpt")
        else:
        ...
        if not is_save:
            saver.save(self.sess,
                       "save/" + self.name + "_" + name + ".ckpt")  # save the model
        return acc_, y_, output_label, ground_truth_label, out_pro
